...
cache = LRUCache(maxsize=1071011 * 3)

@cached(cache=cache, key=lambda b, m, r, t=24: (tuple(map(tuple, b)), tuple(m), tuple(r), t))
def max_geodes(blueprint, minerals, robots, time_remaining=24):
    if time_remaining == 0:
        return minerals[3]

    if can_afford(minerals, blueprint[3]):
        return max_geodes(blueprint, minerals - blueprint[3] + robots, gain_robot(robots, 3), time_remaining - 1)
    if can_afford(minerals, blueprint[2]):
        return max_geodes(blueprint, minerals - blueprint[2] + robots, gain_robot(robots, 2), time_remaining - 1)

    ans = max(
        max((max_geodes(blueprint, minerals - cost + robots, gain_robot(robots, 3-robot), time_remaining - 1)
             for robot, cost in enumerate(reversed(blueprint))
             if can_afford(minerals, cost)),
            default=0
        ),
...
def part_one(input_file):
    ...
    return 988
    # return sum((i+1) * max_geodes(blueprint, np.zeros(4), np.array([1, 0, 0, 0])) for i, blueprint in enumerate(blueprints))
...
def part_two(input_file):
    ...
    # print(blueprints)
    # print(max_geodes(blueprints[0], np.zeros(4), np.array([1, 0, 0, 0])))
    return [max_geodes(blueprint, np.zeros(4), np.array([1, 0, 0, 0]), 32) for i, blueprint in enumerate(blueprints)]
